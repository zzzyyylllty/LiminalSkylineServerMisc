# 阈限天际 Work In Progress

## 简介

> `⚠您正在访问BETA开放测试⚠`
本服务器的部分细节尚未完工，但整体框架运行良好。封闭测试不代表最终成果。祝你好运！
目前只开放到L24，其余项目仍在施工，敬请期待。

**欢迎回到 Maximizer**

本服务器由 **wangyu1023** 与 **AkaCandyKAngel** 主要开发！
当前本服务器正在趋于稳定和走上正轨。

特别鸣谢6年服主 **Kingsthere** 提供的Java/kotlin代码教学，以供阿卡糖能够完成插件。

## 社区
> `我们不再主要使用QQ作为交流渠道。`
QQ is now deprecated.
为了维持服务器交流渠道的安全与稳定，我们在开服后不再对使用QQ的玩家提供任何支持。
请迁移至OOPZ和Discord；QQ将在不久后弃用。

**请尽快加入其他渠道，以免我们弃用或发生意外时，无法联系到我们。**

**已不再提供 QQ 群聊加入途径**

**Oopz**: [Backrooms Maximizer OOPZ](https://oopz.cn/i/mF3xaE)
**Discord**: [阈限天际-LIMINALSKYLINE-DC](https://discord.com/invite/VHs958jJXj)

## 文档

[阈限天际 Manual](https://ucnp9xb186sr.feishu.cn/wiki/CG2OwBGkJiLvSjksCoicvoPQn3d)

## 其他项目

[Sertraline](https://github.com/zzzyyylllty/Sertraline-Hydrochloride)

## 关于旧版地图

这个服务器的**早期版本**是我和几个朋友在2023年创建的旧系列《后室最大化》指令兼容性地图，
但如今已经全部推倒重做，无任何命令方块和函数使用，自定义内容完全依靠 Kotlin 语言的自制插件。
该系列在阿卡糖任职时曾是最受欢迎的后室服务器之一，达到了开服当天32人。
但该地图/服务器的2.0已经永久停止支持。2.1版本已不再由原最大化团队接手主力开发。
若您实在想游玩此旧地图（不推荐），可前往网易MC资源中心搜索“后室最大化”免费下载1/11版本。
我们也欢迎各位玩家对此旧地图进行非开设租赁服/国际服性质的更新。
> （感谢@Cvdx07(xiyu)以及@黑米团子对旧版地图发生矛盾的调解）

**感谢你成为我们的玩家，我们希望你的经历不像我们一样令人遗憾。**
